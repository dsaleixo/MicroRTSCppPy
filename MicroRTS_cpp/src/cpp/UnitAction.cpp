...
UnitAction::UnitAction(const UnitAction &other) {
    ...
    this->r_cache = nullptr;
    if (other.r_cache != nullptr) {
        this->r_cache = new ResourceUsage(*other.r_cache);
    }
}


UnitAction&  UnitAction::operator=(const UnitAction& other) {
    this->type = other.type;
    this->parameter = other.parameter;
    this->x = other.x;
    this->y = other.y;
...
UnitAction::~UnitAction() {
    ...
    this->unitType = nullptr;
  //  if (this->r_cache != nullptr)delete this->r_cache;
}
...
ResourceUsage UnitAction::resourceUsage(Unit &u, PhysicalGameState* pgs) {
    if (this->r_cache != nullptr) {
        
        return ResourceUsage (*this->r_cache);
    }
    ...
    switch (type) {
        case TYPE_MOVE: {
            ...
            this->r_cache->positionsUsed.push_back(pos);
        }
                      break;
        case TYPE_PRODUCE: {
            this->r_cache->resourcesUsed[u.getPlayer()] += this->unitType->cost;
            int pos = u.getX() + u.getY() * pgs->getWidth();
            switch (parameter) {
            case DIRECTION_UP:
                pos -= pgs->getWidth();
